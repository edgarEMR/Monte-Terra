...
if (isset($_POST['accion'])) {
    ...
    if ($_POST['accion'] == 'registrar') {
        ...
        try {
           ...
           header('Location: ../Programar_Pago.php?success=1');

        } catch (PDOException $err) {
            $errorCode = $err->getCode();
            echo '<br>' . $err;
            header("Location: ../Programar_Pago.php?success=0");
    ...
    if ($_POST['accion'] == 'guardar') {
        echo 'GUARDAR' . '<br>';
        foreach ($_POST['pago'] as $pago) {
            echo $pago;
            $procedure = $coneccion->guardarPago($pago);
            $resultado = $procedure->fetch(PDO::FETCH_ASSOC);
        }

        header('Location: ../Programacion_Pago.php?success=1');
    }

}


function insertarPago($pago, $coneccion) {
    $procedure = $coneccion->gestionPagoProgramacion(
        0,
        $pago->getConcepto(),
        $pago->getImporte(),
